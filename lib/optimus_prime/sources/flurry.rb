...
module OptimusPrime
  module Sources
    class Flurry < OptimusPrime::Source
      ...
      #
      # This makes the initial request for the report. The data is not returned. Only a json response is returned.
      # The response should have the url to retrieve the actual resulting data from the request.
      #
      def request_report(from, to)
        raise ArgumentError.new 'Invalid date range' unless from.kind_of? Time and to.kind_of? Time and from < to
